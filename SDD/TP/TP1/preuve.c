...
succes=0 ;
/* Vérification avec l’accesseur estNul *//* vérification de la propriété : estNul(p) = True  estNul(p1) = True  a0 = 0 */
if ( estNul(p)) if ( estNul(p1) && a0 == 0 ) succes=1;
if ( estNul(p1) && a0 == 0) if (estNul(p) ) succes = succes+1;
/*Bilan de la vérification */
if (succes != 2 )
...
if ( Degre(p) = Degre(p1) +1 ) if ( !estNul(p1) )
succes = succes + 1;
if ( !estNul(p1)) if( Degre(p) == Degre(p1) +1 )
succes = succes + 1;
/* Bilan de la vérification */
if (succes !=4)
...
if (!estNul(p1 ) ) for(i=1; i<= Degre(p1)+1 ; i++) if ( Coefficient(p, i) != Coefficient(p1,i-1) ) success = 0 ;
/* vérification de la propriété : estNul(p1) = False  i> Degre(p1)+1  Coefficient(Construire( p1,a0), i) = 0 */
if (!estNul(p1 ) ) for(i= Degre(p1)+2; i<= MaxSize ; i++) if ( Coefficient(p, i) != 0 ) success = 0 ;
